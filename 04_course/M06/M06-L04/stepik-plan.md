...
| 5 | conditional recovery | Only affected situations. Three new learner-facing cases. Complete new attempt and move only forward; old C01 is never revisited. Unavailable source is not replaced by second AI. Successful learners skip | authored cases in `lesson.md` | recovery evidence через natural trace + next self-review | RECOVERY |
| 6 | post-recovery reflection + independent real application | If step5 recovery was used, **after** it locally analyze only repeated situations using the same semantic objects that C01 assessed; successful learner skips this fragment. Then choose one usable checked result and actually finish a small external artifact | natural trace + learner note/document/plan | `M06-L04-E02` | COMPOSITE |
...
| 10 | conditional transfer recovery | If content hint/technical failure compromised transfer, solve new Windows 11 clipboard-history claim independently and move only forward. Optional chat is only support; it never substitutes ground | authored alternative claim + conditional chat | recovery evidence через next self-review | RECOVERY |
| 11 | post-recovery transfer reflection + final boundary | If step10 recovery was used, **after** it locally analyze decision, actual ground/status/action and transferred principle; successful learner skips this fragment. Then close M06 and transition to M07 | natural trace / обычная учебная заметка | — | REFLECTION |
...
Use only the replacement(s) for affected main situation(s), then move forward to step 6, whose first fragment is a post-recovery self-review. Old C01 is never revisited; old work remains practice.
...
No ready URL or method is supplied. After the new attempt learner moves forward to step 11 for a separate post-recovery self-review. Old C03 is never revisited.
...
- step5 has no backward target; step6 begins with post-recovery self-review;
- step10 has no backward target; step11 is post-recovery self-review + final boundary;
