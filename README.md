...
- [production vertical slice M00–M01](04_course/README.md): пять готовых ученических уроков, пять Stepik-планов и 11 Asset ID, произведённых по утверждённым карточкам и Service Acceptance.

Human pilot M00–M01 на настоящих новичках **ещё не проведён** и остаётся обязательным до beta / публичного выпуска. По `D-2026-09-11-PILOT-DEFER` его отсутствие больше не блокирует production последующих модулей: работа продолжается отдельными batch M02–M03, M04–M05, M06 и M07–M08. Поздний pilot может потребовать обратных изменений уже произведённых материалов; до него нельзя считать подтверждёнными людьми длительность, понятность или самостоятельность прохождения.
