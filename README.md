...
Ветка со смёрженным в `main` PR может стать кандидатом только если её текущий SHA точно совпадает с head SHA этого смёрженного PR, после merge не было новых коммитов и прошёл установленный карантин. Ветка без подходящего merged PR может стать кандидатом только если сравнение с `main` показывает `ahead_by = 0` и прошёл отдельный более длинкий карантин. Непосредственно перед реальным удалением workflow повторно проверяет текущий SHA, protection и открытые PR, чтобы не удалить ветку, состояние которой изменилось во время запуска.
