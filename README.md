...
- model-based synthetic pre-pilot отчёты для M00–M05, которые не являются human validation.

Human pilot на настоящих новичках **ещё не проведён** и остаётся обязательным до beta / публичного выпуска. По `D-2026-09-11-PILOT-DEFER` его отсутствие больше не блокирует production последующих модулей: следующий отдельный batch — M06, затем M07–M08. Поздний pilot может потребовать обратных изменений уже произведённых материалов; до него нельзя считать подтверждёнными людьми длительность, понятность или самостоятельность прохождения.
