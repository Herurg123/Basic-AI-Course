...
class BulkStatusTests(unittest.TestCase):
    def test_all_course_status_keeps_golden_read_only_and_marks_placeholder_pending(self) -> None:
        ...
        self.assertEqual(by_id["M00-L03"]["repo_relative_links_pending_asset_route"], [])
        self.assertEqual(status["hard_blockers"], [])
        self.assertTrue(status["asset_route_gate_passed"])
        self.assertEqual(status["next_gate"], "verified-rendering-and-first-upload")
    ...
    def test_unresolved_asset_report_blocks_next_gate(self) -> None:
        ...
        target = next(
            item for item in asset_report["resolutions"] if item["lesson"] == "M06-L02"
        )
