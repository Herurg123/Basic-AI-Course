...
WORKFLOWS = {
    "repository-janitor.yml": {
        "needs": "[cleanup]",
        "purpose": "Проверить рабочие ветки репозитория и выполнить только доказанно безопасную уборку по действующим защитным правилам.",
    },
    "stepik-bulk-status.yml": {
        "needs": "[tests, bulk_status]",
        "purpose": "Провести offline-проверки и read-only сверку статуса всех 21 уроков приватного курса Stepik без записей.",
    },
    "stepik-golden-title-migration.yml": {
        "needs": "[tests, live]",
        "purpose": "Проверить golden-title контракт и при явном owner-dispatch безопасно выполнить только разрешённую миграцию заголовков M00-L01/M00-L02.",
    },
    "stepik-learner-hygiene.yml": {
        "needs": "[tests, live]",
        "purpose": "Проверить learner-facing hygiene и при явном owner-dispatch выполнить только подтверждённые guarded-изменения с read-back и фиксацией состояния.",
    },
    "stepik-private-release.yml": {
        "needs": "[tests, release]",
        "purpose": "Провести полный private-release: offline/preflight проверки, guarded-сборку оставшегося Stepik-состояния, course page, golden-операции и финальную проверку backlog.",
    },
    "stepik-section-position-recovery.yml": {
        "needs": "[tests, live]",
        "purpose": "Проверить и при явном owner-dispatch безопасно восстановить section.position по утверждённому recovery-контракту.",
    },
    "stepik-staging-batch.yml": {
        "needs": "[tests, batch-staging]",
        "purpose": "Проверить и при явном owner-dispatch последовательно собрать все ordinary PENDING lessons приватного Stepik-стейджинга с read-back и durable history.",
    },
    "stepik-staging-build.yml": {
        "needs": "[tests, staging-build]",
        "purpose": "Проверить и при явном owner-dispatch безопасно собрать один выбранный урок приватного Stepik-стейджинга с recovery/read-back защитой.",
    },
    "stepik-uploader.yml": {
        "needs": "[tests, impact, live]",
        "purpose": "Проверить Stepik automation, при merge обновить machine-readable impact, а при owner-dispatch выполнить выбранный read-only или guarded write-режим.",
    },
}
        ...
        with:
          ...
          script: |
            ...
            const statusRu = {
              ...
              failure: 'ошибка',
              cancelled: 'отменено',
              skipped: 'пропущено по условию запуска',
              ...
              startup_failure: 'ошибка запуска runner',
            };
            ...
            const isInfraStep = (name) => {
              ...
              return value === 'Set up job'
                || value === 'Complete job'
                || value.startsWith('Post ')
                || value === 'Сформировать итог запуска на русском';
            };
            ...
            async function errorDetail(job, failedStep) {
              ...
              try {
                ...
                const check_run_id = Number(match[1]);
                ...
                const message = escapeMd(annotation.message).slice(0, 500);
                return `${fallback} GitHub: ${message}`;
              } catch (error) {
                return fallback;
              }
            }
            ...
            else if (relevantJobs.some((job) => !['success', 'skipped'].includes(job.conclusion))) overall = '⚠️ ЧАСТИЧНЫЙ/НЕОПРЕДЕЛЁННЫЙ РЕЗУЛЬТАТ';

            const lines = [];
            lines.push('# Итог GitHub Actions');
            lines.push('');
            lines.push(`**Итог:** ${overall}`);
            lines.push('');
            lines.push(`**Что должен был сделать запуск:** ${escapeMd(purpose) || 'описание цели не передано'}`);
            lines.push('');

            lines.push('## Что должно было быть выполнено');
            if (relevantJobs.length === 0) {
              lines.push('- GitHub не вернул список функциональных jobs.');
            } else {
              for (const job of relevantJobs) {
                lines.push(`- **${escapeMd(job.name)}**`);
                const steps = (job.steps || []).filter((step) => !isInfraStep(step.name));
            ...
            if (successfulJobs.length === 0) {
              lines.push('- Успешно завершённых функциональных jobs нет.');
            } else {
              for (const job of successfulJobs) {
                lines.push(`- ✅ **${escapeMd(job.name)}** — выполнено успешно.`);
                const completedSteps = (job.steps || []).filter((step) => !isInfraStep(step.name) && step.conclusion === 'success');
            ...
            if (notDoneJobs.length === 0) {
              lines.push('- Невыполненных этапов нет.');
            } else {
              for (const job of notDoneJobs) {
                ...
                const incompleteSteps = (job.steps || []).filter((step) => !isInfraStep(step.name) && step.conclusion !== 'success');
      ...
      - name: Проверить все workflow
        ...
        run: |
          ...
          if not reusable.is_file():
              raise SystemExit('Отсутствует обязательный reusable workflow _russian-run-summary.yml')
          ...
          if not instruction.is_file():
              ...
              if 'Обязательная русскоязычная GitHub Actions Summary' not in text:
...
BOOTSTRAP_WORKFLOW = ROOT / ".github/workflows/_bootstrap-russian-summary-migration.yml"
THIS_SCRIPT = Path(__file__).resolve()


def patch_workflow(path: Path, needs: str, purpose: str) -> None:
    text = path.read_text(encoding="utf-8")
    if "  run_summary:" in text:
        return
...
'''
    if marker not in text:
        raise SystemExit("Не найден маркер для вставки раздела 14")
...
def update_docs() -> None:
    pi_readme = ROOT / "00_governance/project-instructions/README.md"
    text = pi_readme.read_text(encoding="utf-8")
    text = text.replace("`project-instructions-v1.4.md`", "`project-instructions-v1.5.md`")
    ...
    if "итоговую русскоязычную Summary" not in text:
        text = text.replace("\nDOCX создаётся", extra + "\nDOCX создаётся", 1)
    pi_readme.write_text(text, encoding="utf-8")
    ...
    text = text.replace("[инструкция проекта v1.4](00_governance/project-instructions/project-instructions-v1.4.md)", "[инструкция проекта v1.5](00_governance/project-instructions/project-instructions-v1.5.md)")
...
def main() -> None:
    ...
    for filename, cfg in WORKFLOWS.items():
        patch_workflow(workflow_dir / filename, cfg["needs"], cfg["purpose"])

    (workflow_dir / "_russian-run-summary.yml").write_text(REUSABLE, encoding="utf-8")
    (workflow_dir / "project-governance.yml").write_text(GOVERNANCE, encoding="utf-8")
    build_v15()
    update_docs()
