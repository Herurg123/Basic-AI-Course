...
def run(args: Any, profile: dict[str, Any]) -> int:
    ...
    The accepted golden fixture is the pre-write live proof. The route may PUT
    only existing step-sources while preserving lesson id, eight step ids and
    positions. Any drift, public state, structural difference or ambiguous
    write fails closed.
    """
    ...
    try:
        ...
        module, target_manifest = legacy._manifest_target(manifest)
        ...
        if int(baseline.get("stepik_lesson_id", -1)) != int(observed.get("stepik_lesson_id", -2)):
            raise GoldenContentRefreshError(
        ...
        for current, expected in zip(_ordered_steps(working), desired_steps, strict=True):
            ...
            after_model = legacy._replace_step(working, step_id=step_id, expected=expected)
            after_fp = legacy.live_lesson_fingerprint(after_model)
        ...
        if final_fp != desired_fp:
            ...
            raise GoldenContentRefreshError(
                f"{TARGET_ID}: final live не совпал с current 8-step canonical desired"
            )
        ...
        (report_dir / "sync-journal.md").write_text(
            "\n".join(
                [
                    ...
                    "",
                    "Only existing step content was eligible for guarded PUT. Issue state may be patched only after final read-back; golden-profile.next.json requires a separate evidence PR.",
                ]
            )
            + "\n",
            encoding="utf-8",
