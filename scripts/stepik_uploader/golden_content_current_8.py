...
def run(args: Any, profile: dict[str, Any]) -> int:
    """Confirm accepted 8-step golden when canonical, live and baseline are exact."""
    ...
    try:
        ...
        learner_rows = [row for row in target_manifest.get("steps", []) if row.get("author_only") is not True]
        if len(desired_steps) != 8 or len(learner_rows) != 8:
            raise GoldenContentMigrationError(
                f"{TARGET_ID}: canonical после принятия fixture должен содержать ровно 8 learner steps"
            )
        ...
        if pending is not None:
            raise GoldenContentMigrationError(
                f"{TARGET_ID}: accepted 8-step golden имеет PENDING; требуется guarded refresh route"
            )
        if not isinstance(baseline, dict):
            raise GoldenContentMigrationError(
                f"{TARGET_ID}: 8-step golden fixture требует confirmed machine baseline"
            )
        if len(baseline.get("step_ids", [])) != 8:
            raise GoldenContentMigrationError(
        ...
        if final_ids != [int(value) for value in baseline.get("step_ids", [])]:
            raise GoldenContentMigrationError(
                f"{TARGET_ID}: live step IDs drifted относительно machine baseline"
            )
        if int(baseline.get("stepik_lesson_id", -1)) != int(lesson["id"]):
            raise GoldenContentMigrationError(
        ...
        confirmed["confirmation_status"] = "NOOP_CONFIRMED"
        next_state = legacy.with_record(state, canonical_id=TARGET_ID, record=confirmed)
        ...
        history_state, recovered_baseline = legacy._history_state(records, live_fp, desired_fp)
        if history_state == "FINAL_PROVEN":
            ...
            confirmed = recovered_baseline
            confirmed["confirmation_status"] = str(
                confirmed.get("confirmation_status") or "NOOP_CONFIRMED"
            )
            next_state = legacy.with_record(state, canonical_id=TARGET_ID, record=confirmed)
            ...
            recorder.ensure_started(
                ...
                state_before={"fixture_fingerprint": legacy._fixture_fingerprint(profile)},
        ...
        legacy.write_json(
            ...
            legacy._event_artifact(identity, confirmed, "NOOP_CONFIRMED"),
        )
